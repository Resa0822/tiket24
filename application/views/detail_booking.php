...
var kode = "<?php echo $fld['transaction_code']; ?>";	
	...
	$('#validateBooking').click(function(){
       ...
       $.ajax({   
           url: "<?=base_url()?>index.php/booking/validate", 
           async: false,
           type: "POST", 
...
switch($statusBooking){
	case 0 : $bkngStts = '<span style="color:#FF0000;font-weight:bold;">Waiting for validate (Payment Confirmation)</span>';break;
...
<?php
	...
	$bkngExpryDte = $bkngExprd;
	$now = date('Y-m-d H:i:s');
	$expryDate = date('Y-m-d H:i:s', strtotime("$bkngExpryDte"));
	if($now < $expryDate && $statusBooking == 0){
?>
<div style="padding: 10px; display: inline-block;">
	<!--<input type="button" name="validateBooking" id="validateBooking" value="<?php echo $bttnVldtVal; ?>" <?php echo $bttnVldtDsbld; ?>  />
	<div id="imgLoadr"></div>-->
	<a href="<?php echo base_url(); ?>index.php/booking/validate/<?php echo $transCode; ?>"><input type="button" name="vldt" id="vldt" value="Validate"  /></a>
</div>	
<?php		
$textExprd = '#Booking expired at <strong>'.$bkngExprd.'</strong>';
	}
	elseif($now > $expryDate && $statusBooking == 0){
$textExprd = '<i style="color: #FF0000;"># <strong>BOOKING EXPIRED ! #</strong></i>';
?>
<div style="padding: 10px; display: inline-block;"><a href="<?php echo base_url(); ?>index.php/booking/delete/<?php echo $transCode; ?>"><input type="button" name="delete" id="delete" value="Delete"  /></a></div>
<?php		
	}
	else{
		$textExprd = '<i style="color: #FF0000;"># <strong>VALIDATED ! #</strong></i>';
?>
<div style="padding: 10px; display: inline-block;">VALIDATED</div>
<?php
	}
?>
...
<div id="pesan" style="display: inline-block;"></div>	
 ...
 <table>
 	...
 	<tr>
 		...
 		<td style="border-bottom: 1px #ccc solid;border-top: 1px #ccc solid;">: <?php echo '<b>'.$transCode.'</b>'; ?><span style="float:right;"><?php echo $textExprd; ?></span></td>
 	</tr>
 	<tr>
 		<td>Booking Date </td>
 	...
 	<tr>
 		...
 		<td style="border-bottom: 1px #ccc solid;">: 
 			...
 			if($now < $expryDate){
				...
				echo '<i style="color: #FF0000;"># <strong>BOOKING EXPIRED ! #</strong></i>';
			}
 			 ?>
 		</td>
 	</tr>
 		...
 		<td>
			...
			<tr>
  			...
  			<tr>
	 			...
	 			<td style="border-bottom: 1px #ccc solid;">: <?php echo '<b>'.$fld['currency_code'].'</b>&nbsp;'.$fld['total_sale_price_amount'].',-'; ?></td>
  			</tr>
			<tr>
 	...
 	<tr>
 		...
 		<td> 
 			<table>
				...
				-->
				<tr>
					<td style="width: 200px;border-bottom: 1px #ccc solid;">Adult Price</td>
					<td style="border-bottom: 1px #ccc solid;">: <?php echo '<b>'.$fld['currency_code'].'</b>'.'&nbsp;'.$fld['adult_sale_price'],' ,-'; ?></td>
				</tr><tr>
					<td style="width: 200px;border-bottom: 1px #ccc solid;">Child Price</td>
 	...
 	<tr>
 		...
 		<td>
			<table>
				...
				$ntnltyTL = $getTrvlrLdr['nationality_id'];
				$qryNtnltyTL = mysql_query("SELECT * FROM country WHERE idx='$ntnltyTL' ") ;
