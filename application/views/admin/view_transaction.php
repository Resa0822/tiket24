 ...
 <!--       <colgroup>
		...
		?>
			...
			<tr class="k-alt" role="row" style="text-align: center;" >
				...
				<td class="" role="gridcell" title="<strong><?php  echo $nmMthds; ?></strong><br /> <strong�<strong>Grade : </strong> 3 <strong>Status : </strong> Good">
				<?php
				if($usrID !== '0'){
					$qry = $this->db->get_where('users', array('users_id' => $usrID));
					foreach($qry->result() as $rows){
					  $usrNme = $rows->username;
					  $usrRleId = $rows->role_id;
					}
				}
				else{
					$usrNme = 'Guest';
				...
				if($pymntMthdID == 'MP0001'){
				?>
					<div style="font-size:11px;"><a href="<?php echo $voucherLink; ?>" target="_blank">Cetak</a></div>
				<?php
				}
