...
<table class="form">
    <input id="kode_packages" type="hidden" title="Enter Name of packages." size="50" value="<?php echo $isi->kode; ?>" name="kode"></input>
		...
		$ar_countries[$isi->country] = $ar_countries[$isi->country]; }
		else{
		$ar_countries[$isi->country] = 'Please Select';
		}
		?>
		<td class="field">
			<?php echo form_dropdown('country', $ar_countries, $isi->country, 'id="country" name="country"'); ?>
<!--		<select id="country_id" title="Select Country" size="1" name="negara" style="width:50%">
	...
	<tr class="form-row">
		<td >City	: 	</td>
		 ...
		 $ar_cities[$isi->city] = $ar_cities[$isi->city]; }
		 else{
	 	 $ar_cities[$isi->city] = 'Please Select';
	 	 }?>
		<td class="field">
			<?php echo form_dropdown('city', $ar_cities, $isi->city, 'id="cities" name="cities"'); ?>
...
-->
	...
	<tr class="form-row">
        ...
        <td class="field">
        	...
        	$crrncy =  "$isi->currency"; 
        	?>
        	<select name="currency" style="width:40%; height:auto;" >
        	<?php 
				if($crrncy == 'IDR'){
					
			?>
			<option value="SGD" > SGD (Singapore Dollar)</option>
        	<option value="IDR" selected = "selected" > IDR (Indonesia Rupiah)</option>
			<?php
				}
				else{
			?>
        	...
        	<!--
        	<select name="Currency" style="width:20%; height:auto;">
	...
	<div class="button-container" style="float:right;"><input type="submit" name="Save" value="Save"/>&nbsp;
		<input type="reset" name="Reset" value="Retur to API">&nbsp;<input type="button" onClick="history.go(-1)" value="Back" />&nbsp;</div>
</div>
</fieldset>
