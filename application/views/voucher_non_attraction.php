...
foreach($qryPckgs->result() as $rowFld){
	...
	$tour_name = $rowFld->nama;//$rowFld->tour_name;
	$cnfrmtnCode = $rowFld->api_confirmation_number;
	$pckupPnt = $rowFld->pickup_dropoff;
	$pckgeDesc = $rowFld->desc;
	$pckgCde = $rowFld->kode;
}
//select traveler leader info
	...
	foreach($qrySlctTC->result() as $fldTC ){
		...
		$titleTC = $fldTC->title;
		$cntctMbleNmbr = $fldTC->contact_number;
	...
	<div style="font-size: 10px;">
		<table>
			<tr>
				...
				<td>: <?php echo $titleTL.' '.ucwords($namaTL); ?></td>
			</tr>
			<tr>
				<td style="width: 150px;">Contact Travel Party </td>
				<td>: <?php echo $titleTC.' '.ucwords($namaTC); ?></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td> : <?php echo $pckUpPntDrpOff; ?></td>
			</tr>
			-->
		</table>
	</div>
	<div style="border-bottom:1px #000 solid; width: 100%;">&nbsp;</div><br />
	<table id="tblNonThemePark" style="font-size: 10px;">
		<tr>
			<td colspan="2" style="padding-bottom: 5px;"><u><b>TOUR / ATTRACTION HIGHLIGHTS :</b></u></td>
		</tr>
		<tr>
			<td colspan="2">
			<?php echo $pckgeDesc; ?>dfdsfsdf
			</td>
		</tr>
		<!--
		<?php 
