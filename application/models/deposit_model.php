...
class Deposit_model extends CI_Model
{
	...
	function add_deposit()
	{
		...
		$date = date('d/m/y');
		$k = '';
		$qry = $this->db->get_where('deposit', array('id' => $id));
		if($qry->num_rows() == '0'){
		...
		foreach ($query->result() as $row)
		{
		  $nominal = $row->top_up;
			...
			$qry = $this->db->get_where('reseller_profile', array('id' => $id));
			foreach($qry->result() as $row){
				$usrName = $row->first_name.' '.$row->last_name;
			}
			echo '<script>if(confirm("Deposit setting for this user ('.$usrName.'), is already exist !")) window.location = "'.base_url().'index.php/deposit/view";</script>';
		}
		/*
		$role_id = $this->input->post('roles');
		$id = $this->input->post('name');
	...
	function get_deposit_balance($depositID){
	...
	function edit_deposit($id)
	{
		...
		foreach ($query->result() as $row)
		{
		  $nominal = $row->top_up;
			...
			$data1 = array('top_up'=>$nominal - $nml);
