...
function displayCheckprice_booking_form(){
	...
	$WSDL = "http://packages.asiatravel.com/PackagePartnerWSv2/ActivityWS.asmx?wsdl";//"http://packages.asiatravel.com/PackagePartnerWSv2/ActivityWS.asmx?wsdl";
	$konek = FALSE;
	/* =====================check connection to API server start here===================== */
	try{
	    $client = new SoapClient("$WSDL");
	}catch (Exception $e) { // Do NOT try and catch "Exception" here
		$konek = TRUE;
	}
	/* =====================check connection to API server end here===================== */
if($konek !== TRUE){
...
/* ==================package from API start here========================= */	
	...
	*/	
	...
	$xml->registerXPathNamespace('ns', 'http://tempuri.org/');
	 ...
	 /* ==========================Currency From Front======================= */
	if($this->session->userdata('sess_conversi') != 0 or
		$this->session->userdata('sess_currency') != '' or
			$this->session->userdata('sess_currency') != null){
				...
				if($diskon == 0){
					...
					/* if($this->session->userdata('sess_conversi') != 0 or
							$this->session->userdata('sess_currency') != '' or
								$this->session->userdata('sess_currency') != null){
									$Currency = $curr;
									$FsalePriceAdult = round((( $salePriceAdult * $conv_sgd ) / $conv), 2);
									$FsalePriceChild = round((( $salePriceChild * $conv_sgd ) / $conv), 2);
									$FtotalAdultSalePrice = round((( $totalAdultSalePrice * $conv_sgd ) / $conv), 2);
									$FtotalChildSalePrice = round((( $totalChildSalePrice * $conv_sgd ) / $conv), 2);
									$FtotalPackageSalePrice = round((( $totalPackageSalePrice * $conv_sgd ) / $conv), 2);
								}else{	
									$FsalePriceAdult = $salePriceAdult ;
									$FsalePriceChild = $salePriceChild ;
									$FtotalAdultSalePrice = $totalAdultSalePrice ;
									$FtotalChildSalePrice = $totalChildSalePrice ;
									$FtotalPackageSalePrice = $totalPackageSalePrice ;
								} */
				}
				else{
					 ...
					 * */
					/* ==========================Currency From Front======================= */
					/* echo $this->session->userdata('sess_conversi');
					if($this->session->userdata('sess_conversi') != 0 or
							$this->session->userdata('sess_currency') != '' or
								$this->session->userdata('sess_currency') != null){
									$Currency = $curr;
...
echo '
	...
	$errPckgID = $packageID;
					$usrAgent = $this->agent->browser().' at '.$this->input->ip_address();
					$errValue = 'Cannot connect to API Server';
					$this->errorlog_model->save_error_toDB($errMethod, $errPckgRefNo, $errPckgID, $errValue, $usrAgent);
				/* ==============catch error for method SearchActivityPrice end============= */
	echo '<div><div style="display: block; margin-left: auto; margin-right: auto; text-align:center;width:100%;padding:3px;font-weight:bold;color:#FF0000">'.$errMsg.'</div></div>';	
}	
