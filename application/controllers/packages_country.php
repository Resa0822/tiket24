...
class packages_country extends CI_Controller
{
	...
	function searching(){
		...
		$srch = $pecah[0];
		redirect('packages_country/search_result/q/'.$srch, 'resfresh');
	}
	function search_result(){
		...
		$config['base_url'] = base_url().'index.php/packages_country/search_result/q/'.$txtSearch;
		$config['total_rows'] = $this->packages_country_model->search_package_country_records_count($txtSearch);
		$config['per_page'] = 10;
		...
		$data['text'] = $this->packages_country_model->search_package_country($config["total_rows"],$page,$txtSearch);
	...
	function edit($id)
	{
		if($this->input->post('country_iso'))
		{
			...
			$data['update']=$this->packages_country_model->edit_packages_country($id,$nama_file);
			$this->load->vars($data);
			$isi=$this->tank_auth->get_user_role_id();
			if($isi==1)
			{ 
	...
	function order_package()
	{
		$id = $this->input->post('id');
		$tOrder = $this->input->post('content');
			$date = date('Y-m-d');
			$data = array(
						'orders'=>$tOrder,
						'date_ch'=>$date
