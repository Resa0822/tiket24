...
function all_booking_history_by_period(){
		...
		if($userRoleID == '1'){
			...
			$this->db->where('user_role', $userRoleID);
			$qryBkngPkg = $this->db->get();
		}
...
function booking_form2(){
	...
	$config = array(
			   ...
			   array('field'   => 'txtArrivDate', 'label'   => 'Arrival Flight Date', 'rules'   => 'required'),
			   array('field'   => 'txtDepartDate', 'label'   => 'Departure Flight Date', 'rules'   => 'required'),
...
$ntnltyAdlt = explode("-",$adultNationality[$i]);
$hasilAdlt[] = array('Guest' => array(
									'Title' => $adultSalutation[$i], 
									'FirstName' => $adultFirstName[$i], 
									'LastName' =>  $adultLastName[$i],
									'PassengerType' => $adultTravelerType[$i],
									'Age' =>  $adultAge[$i],
									'PassportNumber' =>  $adultPassportNumber[$i],
									'DOB' =>  $adultDOB[$i],
									'PassportExpiryDate' =>  $adultPassportExpiredDate[$i],
									'PassportIssuanceCountry' => $adultPassportIssuanceCountry[$i],
									'NationalityID' =>  $ntnltyAdlt[0]
									),	
								);
}	
$arrAdultGuest = array($hasilAdlt);

$arrChildGuest = array();
for($i=0;$i <= $childLoopEnd;$i++){
$ntnltyChld = explode("-",$childNationality[$i]);
$hasilChld[] = array('Guest' => array(
									'Title' => $childSalutation[$i], 
									'FirstName' => $childFirstName[$i], 
									'LastName' =>  $childLastName[$i],
									'PassengerType' =>  $childTravelerType[$i],
									'Age' =>  $childAge[$i],
									'PassportNumber' =>  $childPassportNumber[$i],
									'DOB' =>  $childDOB[$i],
									'PassportExpiryDate' =>  $childPassportExpiredDate[$i],
									'PassportIssuanceCountry' =>  $childPassportIssuanceCountry[$i],
									'NationalityID' =>  $ntnltyChld[0]
									),	
								);
}	
$arrChildGuest = array($hasilChld);

$jmlChldPax = count($childAge); 
if($jmlChldPax !== 0){
	$arrGuestResult = array_merge($arrAdultGuest, $arrChildGuest);
}
else{
	$arrGuestResult = $arrAdultGuest;
}


$qryPckupPont = $this->db->get_where('pickup_point', array('hotel_code' => $pickupDropoff));
foreach($qryPckupPont->result() as $fldPckupPnt){
		$PickupPointID = $fldPckupPnt->hotel_code;
		$PickupDropOffPoint = $fldPckupPnt->hotel_name;
}
		$ntnltyTL = explode("-",$nationalityTL);
		$ntnltyTC = explode("-",$nationalityCodeTC);
		...
		$qryGetDetailsTourBooking = $this->db->get_where('tours', array('API_packages_id' => $packageID));
		...
		$dataBooking = array(
			...
			'PickUpDropOffHotel'=> array(
										...
										'PickupDropOffPoint' => $PickupDropOffPoint
										),
			'LeadTravelerInfo'=> array(
										...
										'NationalityID' => $ntnltyTL[0]
										),
			'GuestDetails' => $arrGuestResult,
			'FlightInfo'=> array(
			...
			'ContactInfo'=> array(
								  ...
								  'NationalityID' => $ntnltyTC[0],
								  'NationalityCode' => $ntnltyTC[1]
							  ),
		...
		foreach ($xmlBooking->xpath('//ns:BookActivityResult') as $itemBooking)
		{
			...
			$BookingStatus = $itemBooking->BookingStatus;
			$bkngConfirmationNumber = $itemBooking->ConfirmationNumber;
			$bkngTotalPrice = $itemBooking->TotalPrice;
				...
				$travContactInfoNew = array(
									  ...
									  'city_code' => $cityTC,
									  'nationality_id' => $ntnltyTC[0],
									  'nationality_code' => $ntnltyTC[1]
				...
				$travLeaderInfoNew = array(
									  ...
									  'nationality_id' => $ntnltyTL[0],
									  'nationality_code' => $ntnltyTL[1],
									);	
				$this->db->insert('traveler_info', $travLeaderInfoNew); 
				/* ===========insert adult traveler info=============== */
				for($i=0;$i <= $adultLoopEnd;$i++){
				$adltNtnlty = $adultNationality[$i];
				$ntnltyAdlt = explode("-", $adltNtnlty);
				$adultInfoNew = array(
									  'transaction_code' => $transCode,
									  'usid' =>$usid,
									  'API_packages_id' => $packageID ,
									  ...
									  'nationality_id' => $ntnltyAdlt[0],
									  'nationality_code' => $ntnltyAdlt[1],
									);	
				...
				for($i=0;$i <= $childLoopEnd;$i++){
				$chldNtnlty = $childSalutation[$i];
				$ntnltyChld = explode("-", $chldNtnlty);
				$childInfoNew = array(
									  'transaction_code' => $transCode,
									  'usid' =>$usid,
									  'API_packages_id' => $packageID ,
									  ...
									  'nationality_id' => $ntnltyChld[0],
									  'nationality_code' => $ntnltyChld[1],
									);	
			 ...
			 $dataUpdt = array(
			   ...
			   'api_confirmation_number' => $bkngConfirmationNumber
            );
...
/* payment with bank transfer start */
else{			
			$qrySlct = mysql_query("SELECT * FROM book_packages WHERE usid='$usid' ");
			$flds = mysql_fetch_array($qrySlct);
			$adultTot = $flds['adult'];
...
/* payment with bank transfer end */
		}
		/*
	$usid =;
	$activityID=;
	...
	$config = array(
		...
		else
		{
			if ($this->tank_auth->is_logged_in()){
				...
				/*  member and reseller booking start here */
				$usid = $this->input->post('usid');	
				$pid = $this->input->post('pid');	
				$paymentMethod = $this->input->post('paymethod');	
				$isRadeem = $this->input->post('radeemMyPoint');
				...
				/* ===========insert adult traveler contact info=============== */
				$adltTrvlrCntctNtnlty = explode("-", $nationalityCodeTC);
				$travContactInfoNew = array(
									  'transaction_code' => $transCode,
									  'usid' => $usid,
									  'API_packages_id' => $packageID ,
									  ...
									  'country_code' => $countryTC,
									  'city_code' => $cityTC,
									  'nationality_id' => $adltTrvlrCntctNtnlty[0],
									  'nationality_code' => $adltTrvlrCntctNtnlty[1],
									   
									);	
				$this->db->insert('traveler_info', $travContactInfoNew); 
				
				/* ===========insert adult traveler leader info=============== */
				$adltTrvlrCntctNtnlty = explode("-", $nationalityTL);
				$travLeaderInfoNew = array(
									  'transaction_code' => $transCode,
									  'usid' =>$usid,
									  'API_packages_id' => $packageID ,
				...
				/* ===========insert adult traveler info=============== */
				for($i=0;$i <= $adultLoopEnd;$i++){
					$adltTrvlrNtnlty = explode("-", $adultNationality[$i]);
					$adultInfoNew = array(
										  'transaction_code' => $transCode,
										  'usid' =>$usid,
										  'API_packages_id' => $packageID ,
										  ...
										  'passport_expired_date' => $adultPassportExpiredDate[$i],
										  'passport_issuance_country' =>  $adultPassportIssuanceCountry[$i],
										  'nationality_code' => $adltTrvlrNtnlty[1],
										);	
				...
				/* ==========insert child traveler info============= */
				for($i=0;$i <= $childLoopEnd;$i++){
					$childTrvlrNtnlty = explode("-", $childNationality[$i]);
					$childInfoNew = array(
										  'transaction_code' => $transCode,
										  'usid' =>$usid,
										  'API_packages_id' => $packageID ,
										  ...
										  'passport_expired_date' => $childPassportExpiredDate[$i],
										  'passport_issuance_country' =>  $childPassportIssuanceCountry[$i],
										  'nationality_code' => $childTrvlrNtnlty[1],
										);	
				...
				$dataUpdt = array(
					...
					$this->db->update('book_packages', $dataUpdt); 
		         	$this->session->unset_userdata('bookingUSID');
			 /* member and reseller booking end here */
			}
			else{
			/* guest booking start here */
				...
				/* check payment method start here*/
				if($paymentMethod == 'MP0001'){
					$this->db->select('*');
					$this->db->from('book_packages');
					$this->db->where('usid', $usid);
					$qryBkng = $this->db->get();
					foreach($qryBkng->result() as $bkngRow){
						$trBkngCde = $bkngRow->transaction_code;
						$pckgID = $bkngRow->API_packages_id;
						$adultPax = $bkngRow->adult;
						$childPax = $bkngRow->child;
						$actvtyID = $bkngRow->API_packages_id;
						$adultLoopEnd = $adultPax - 1;
						$childLoopEnd = $childPax - 1; 
						$travDate = $bkngRow->travel_date;
						$bkngUSID = $bkngRow->usid;
						
					}
					$qryGetDetailsTour= $this->db->query("SELECT * FROM tours WHERE API_packages_id='$pckgID' ");
					...
					$arrChildAge = array();
					$chldAge = array();
					$qrySlctTrvlrChldAge = mysql_query("SELECT age FROM traveler_info WHERE transaction_code='$trBkngCde' AND isChild='1' AND traveler_type='33' ");
					while ($fldTrvlrChldAge = mysql_fetch_array($qrySlctTrvlrChldAge)){
						$chldAge[$i] = $fldTrvlrChldAge['age'];
						$i++;
					}
					$arrChildAge = array('Age' => $chldAge);
				/* payment with credit card start here */
					...
					$Data = array(
						...
						foreach ($xml->xpath('//ns:SearchActivityPriceResult/ns:ActivitiePrice') as $item)
						{
							...
							$qryGetDetailsTourBooking = $this->db->select('*')->from('tours')->where('API_packages_id', $pckgID)->get();
							foreach ($qryGetDetailsTourBooking->result() as $fldBking)
							{
								$tridBkng = $fldBking->tour_id;
								$tourCtgry = $fldBking->tour_category;
								$trTpe[] = array($fldBking->tour_type);
							...
							$qryPickupPoint = $this->db->get_where('pickup_point',array('hotel_code' => $pickupDropoff));
							foreach($qryPickupPoint->result() as $fldPP){
								$PickupPointID = $fldPP->hotel_code;
								$PickupDropOffPoint = $fldPP->hotel_name;
							}
							$ntnltyTL = explode("-", $nationalityTL);
							$ntnltyTC = explode("-", $nationalityCodeTC);
							...
							foreach ($xmlBooking->xpath('//ns:BookActivityResult') as $itemBooking)
							{
								$bookUsid = $itemBooking->USID;
								$bookPackageId = $itemBooking->ActivityID;
								$bookStatus = $itemBooking->BookingStatus;
								$bookConfirmNumber = $itemBooking->ConfirmationNumber;
								$bookCurrency = $itemBooking->Currency;
								$bookTotPrice = $itemBooking->TotalPrice;
								
								/* ===========insert adult traveler contact info=============== */
								$travContactInfoNew = array(
													  'transaction_code' => $trBkngCde,
													  'usid' => "$usid",
													  'API_packages_id' => $bookPackageId ,
													  'isLeader' => '0',
													  'isContact' => '1',
													  'isAdult' => '0',
													  'isChild' => '0',
								...
								$dataUpdt = array(
						               ...
						               'departure_flight_date' => "$departDate",
						               'pickup_dropoff' => "$pickupDropoff",
						               'api_booking_status' => "$bookStatus",
						               'api_confirmation_number' => "$bookConfirmNumber",
						               );
								...
								echo 'result '.$bookUsid.'<br />'.
								$bookPackageId.'<br />'.
								$bookStatus.'<br />'.
								$bookConfirmNumber.'<br />'.
								$bookCurrency.'<br />'.
								$bookTotPrice.'<br />'.
								json_encode($trTpe);
							}
				 ...
				 /* payment with credit card end here */	
				}
				}
				else
				{
				/* payment with bank transfer start here */
					$qrySlct = mysql_query("SELECT * FROM book_packages WHERE usid='$usid' ");
					$flds = mysql_fetch_array($qrySlct);
					$adultTot = $flds['adult'];
					$childTot = $flds['child'];
					...
					$dataUpdt = array(
						...
						$this->db->update('book_packages', $dataUpdt); 
			         	$this->session->unset_userdata('bookingUSID');
				/* payment with bank transfer end here */		
				}
				/* check payment method end here*/
