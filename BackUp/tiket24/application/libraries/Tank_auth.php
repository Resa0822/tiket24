...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				$md5_passsword = md5($password);
				
				//if ($hasher->CheckPassword($password, $user->kunci)) {		// password ok
				
				if ($md5_passsword == $user->kunci) {		// password ok
				
					if ($user->banned == 1) {									// fail - banned
						$this->error = array('banned' => $user->ban_reason);

						//create login history banned
						$this->ci->users->create_history($user,'','login banned');

					} else {
						//get roles
						$user_role = $this->ci->users->get_user_role($user->role_id);
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							//create login history not activated
							$this->ci->users->create_history($user,'','login not activated');

						} else {												// success
							if ($remember) {
								$this->create_autologin($user->users_id);
							}
							...
							//create login history login
							$this->ci->users->create_history($user,'','login');
					...
					//create login history fail
					$this->ci->users->create_history($user,'','login failed wrong password');
				}
			} else {															// fail - wrong login
				$this->increase_login_attempt($login);
				$this->error = array('login' => 'auth_incorrect_login');
				//create login history fail
				$this->ci->users->create_history('',$login,'login fail wrong login');
			}
		}
		return FALSE;
	}
	...
	function logout()
	{
		...
		$login = array(
			'users_id'	=> $this->ci->session->userdata('user_id'),
			'username'	=> $this->ci->session->userdata('username')
			);

		$user = (object) $login;

		//create login history logout
		$this->ci->users->create_history($user,'','logout');
	 ...
	 * @return	bool
	 */
	function is_logged_in($role = '',$activated = TRUE)
	{
		if($role != ''){

			if ($this->get_user_role_id() == 1) return TRUE; 

			return ($this->get_role() == $role)? TRUE : FALSE;
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $role_id, $email_activation)
	{
	...
	function forgot_password($login)
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_login($login))) {
				...
				$this->ci->users->set_password_key($user->users_id, $data['new_pass_key']);
				return $data;
	...
	function reset_password($user_id, $new_pass_key, $new_password)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
				...
				$md5_passsword = md5($new_password);
				...
				if ($this->ci->users->reset_password(
					...
					$this->ci->user_autologin->clear($user->users_id);
	 ...
	 * @return	bool
	 */
	function new_login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				// Does password match in database?
						
				if ($this->check_password($password, $user->kunci)) {		// password ok
	...
	private function check_password($password, $password_db){
		
		$password_encrypt = $this->fnEnkrip($password);
	 ...
	 * @return	bool
	 */
	function reset_new_password($user_id, $new_pass_key, $new_password)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
				...
				$hashed_password = $this->fnEnkrip($new_password);
	...
	private function fnEnkrip($pass){
		
		$pass = trim($pass);
		
        $pass = str_pad($pass, 20, ' ', STR_PAD_RIGHT);
		...
		if (strlen($pass) > 0 ){
			...
			for ($i=1; $i <= strlen($pass); $i++) {
				...
				$new_pass .= chr(ord(substr($pass,$i-1,1)) + 14 + $i);
	...
	private function fnDekrip($pass){
