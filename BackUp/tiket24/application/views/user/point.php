...
<div style="clear:both;"></div>
	 ...
	 <br />
	 <form action="<?php echo base_url().'index.php/transaction/point_by_period'; ?>" enctype="multipart/form-data" method="post">
...
<div id="grid" class="k-grid k-widget k-secondary k-reorderable" data-role="grid" style="">
    <div class="k-grouping-header" data-role="droptarget"> &nbsp; Point History 
		<span class="k-pager-info k-label">
			<?php
			$usrIdRole = $this->tank_auth->get_user_role_id();
			if($this->tank_auth->get_user_role_id() !== '1'){
			?>
			<a href="<?php echo base_url()?>index.php/transaction/point_history/" target="_blank"><img style="width:60px;height:30px;"src="<?php echo base_url()?>asset/images/icon/pdf-download.png"/></a>
			<?php	
			}else{
			?>
			<a href="<?php echo base_url()?>index.php/transaction/all_point_history/" target="_blank"><img style="width:60px;height:30px;"src="<?php echo base_url()?>asset/images/icon/pdf-download.png"/></a>
			<?php 
			}
			?>
		</span>
 ...
 <!--       <colgroup>
        ...
        <thead class="k-grid-header">
            <tr>
                ...
                <th class="k-header k-with-icon" data-title="Date" data-field="packagess_date" role="columnheader" data-role="sortable">
                    ...
                    <a class="k-link" href="#">Earned</a>
                </th>
                <th class="k-header k-with-icon" data-title="Date" data-field="packagess_date" role="columnheader" data-role="sortable" style="width:50px;">
                    <a class="k-header-column-menu" href="#" tabindex="-1">
                        <span class="k-icon k-i-arrowhead-s"></span>
                    </a>
                    <a class="k-link" href="#">Redeemed</a>
                </th>
		...
		foreach($text as $row)
		{
			...
			$pointEarned = $row->point_transaction;
			$pointRademed = $row->pointRadeem;
			$this->db->select('*');
		...
		?>
			...
			<tr class="k-alt" role="row" style="text-align: center;" >
				...
				<td class="" role="gridcell" title="<strong><?php echo $pointEarned; ?></strong>"><?php echo $pointEarned; ?></td>
				<td class="" role="gridcell" title="<strong><?php echo $pointRademed; ?></strong>"><?php echo $pointRademed; ?></td>
