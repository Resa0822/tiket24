...
class APIhandler extends CI_Controller {
	...
	function __construct()
	{
		...
		$connect = "http://ws.asiatravel.net/PartnerPackageWS/ActivityWS.asmx?wsdl";
		
	}
	
	function API_update_highlight_important_notes(){	
		/* tours highlight and important notes start */
		$WSDL = $connect;
		...
		$this->db->select('city_iso');
		//$this->db->where('city_iso', $name); 
		$query = $this->db->get('packages_city');
		foreach ($query->result() as $row)
		{
		    $ctyCode = $row->city_iso;
			$dataImprtnNt = array(
		    		'CityCode' => "$ctyCode" 	
			);
			$Response = $SOAP->GetTourInfoImportantNotes($dataImprtnNt);
			...
			{
			     $trID = $item->TourID;
				 $trHghlght = $item->TourHighlights;
				 $trImprtnNts = $item->ImportantNotes;
				 
				 $qrySlctNts = mysql_query("SELECT * FROM tour_info WHERE tour_id='$trID' ");
				 $isAdaNts=mysql_num_rows($qrySlctNts);
				 $fieldNts=mysql_fetch_array($qrySlctNts);
				 $idTble = $fieldNts['id']; 
				 if($isAdaNts == 0){
					  $dataQry = array(
									   'tour_id' => "$trID",
									   'tour_highlights' => "$trHghlght",
									   'tour_inportant_notes' => "$trImprtnNts",
							 );
							$this->db->insert('tour_info', $dataQry); 
				}
				 else{
				 	$dataQry = array(
									  'tour_id' => "$trID",
						...
						$this->db->where('id', $idTble);
						$this->db->update('tour_info', $dataQry); 
				 }
			}
		}

		
		/* tours highlight and important notes end */
	
	}
	
	function API_update_packages_country_city_tours(){
		ini_set( "soap.wsdl_cache_enabled", 0 );
		$WSDL2 = $connect;
		//$WSDL2 ="http://ws.asiatravel.net/PartnerPackageWS/ActivityWS.asmx?wsdl";
		//$WSDL1 ="http://packages.asiatravel.com/PackagePartnerWSv2/ActivityLookUp.asmx?WSDL";
		///$WSDL1 ="http://packages.asiatravel.com/PackagePartnerWSv2/ActivityLookUp.asmx?WSDL";
		$SOAP1 = new SoapClient($WSDL1, array('trace' => true));
		$WSDL2 ="http://ws.asiatravel.net/PartnerPackageWS/ActivityWS.asmx?wsdl";
		//$WSDL2 ="http://packages.asiatravel.com/PackagePartnerWSv2/ActivityWS.asmx?wsdl";
		///$WSDL2 ="http://ws.asiatravel.net/PartnerPackageWS/ActivityWS.asmx?wsdl";
		...
		{
		//	echo htmlspecialchar($item2->ActivityInclusive);
		...
		//	$activityInclusive = mysql_real_escape_string(htmlspecialchar($activityInclusive1));
		...
		//Reset publish package
			
			$qryPckge = $this->db->get_where('packages', array('API_packages_refno'=>"$ActivityRefNo", 'API_packages_id'=>"$ActivityID"));
			$publ = array(
				'publish' => '0'
			);
			$this->db->where_in(array('isFromAPI' => '1', 'packages_id' => "$idPckgs", 'API_packages_refno' => "$ActivityRefNo",'API_packages_id' => "$ActivityID",'kode' => "$ActivityRefNo"));
...
/* =============================================================================== */   
	...
	function update_pickuppoints_by_dbPackages(){
	ini_set( "soap.wsdl_cache_enabled", 0 );
	$WSDL = $connect;
		//$WSDL ="packages.asiatravel.com/PackagePartnerWSv2/ActivityLookUp.asmx?WSDL";
		///$WSDL ="http://packages.asiatravel.com/PackagePartnerWSv2/ActivityLookUp.asmx?WSDL";
		$SOAP = new SoapClient($WSDL, array('trace' => true));
		...
		$qry = $this->db->query("SELECT API_packages_id FROM packages");
		foreach ($qry->result() as $row)
		{
		  $packagesID = $row->API_packages_id;
		  
		    $Data = array(
						   'ActivityID' => $packagesID,
						  );
			$Response = $SOAP->GetPickupPointList($Data);
			$string = $SOAP->__getLastResponse();
			$xml = simplexml_load_string($string);
			$xml->registerXPathNamespace('soap', 'http://schemas.xmlsoap.org/soap/envelope/');
			$xml->registerXPathNamespace('ns', 'http://tempuri.org/');
			foreach ($xml->xpath('//ns:GetPickupPointListResult/ns:PickUpPointList/ns:PickUpPoint') as $item)
			{
				$hotelCode = $item->HotelCode;
				$hotelName = $item->HotelName;
				$nmHotel = str_replace("'"," ",$hotelName);
				$qryChk = $this->db->query("SELECT * FROM pickup_point WHERE API_packages_id='$packagesID' ");
