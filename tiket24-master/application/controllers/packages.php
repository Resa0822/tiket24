...
class Packages extends CI_Controller
{
	...
	{
		$qryCntry = $this->db->get('packages_country');
		$countries = array();
		if($qryCntry ->result()){
			foreach($qryCntry ->result() as $country){
...
*/	 
	...
	function details(){
		$pckgId = $this->uri->segment(3);
		$data['listsPckgByID'] = $this->packages_model->get_packages_by_tbl_id($pckgId);
		$data['pagecontent'] = "admin/view_detail_packages";
		$this->load->vars($data);
		$this->load->view('template');
	}
	function searching(){
		$txtSearch = $this->input->post('txtSearch');
		$srchFilter = $this->input->post('searcFilter');
		redirect('packages/search_result/q/'.$txtSearch.'/f/'.$srchFilter, 'resfresh');
	}
	function search_result(){
		$txtSearch = $this->uri->segment(4);
		$srchFilter = $this->uri->segment(6);
		//redirect('packages/search_result/q/'.$txtSearch.'/f/'.$SrchFilter, 'resfresh');
		
		$data['aktif']=2;
		$pge = $this->uri->segment(7);
		//count total rows of packages list
		$config['base_url'] = base_url().'index.php/packages/search_result/q/'.$txtSearch.'/f/'.$srchFilter;
		$config['total_rows'] = $this->packages_model->search_package_records_count($srchFilter, $txtSearch);
		$config['per_page'] = 10;
		...
		$data['total_rows'] = $config['total_rows'] ;
		$data['title'] = 'List packages';
		$data['text'] = $this->packages_model->search_package($config["total_rows"],$page,$srchFilter,$txtSearch);
		$data['pagecontent'] = "admin/view_packages";
		$this->load->vars($data);
		$this->load->view('template');
	}
	...
	{
		$data['reset']=$this->packages_model->reset_order();
		$this->load->vars($data);
		$isi=$this->tank_auth->get_user_role_id();
		redirect('packages/view','refresh'); 
