...
/*	$curr = $this->session->userdata('sess_currency');
	if($this->session->userdata('sess_currency') !== 0)
	{	$conv = $this->session->userdata('sess_conversi'); }
	else
	{	$conv = 1; }
	$conv_sgd = $this->session->userdata('sess_conv_sgd');*/
	//echo "<script type='text/javascript'>alert(' lalal.$curr.lalala ');</script>";
?>
<style>
...
<div class="content detail">
    ...
    <h1 class="white" >
	<?php
//   var_dump(is_null($city));
//   if (is_array($city))
//	{
	/*	
	if($tanda == 1){ echo "Tours & Special Packages"; }
	elseif($tanda == 2) { 
		if(is_null($city) == false){$isi = $city->country_name ; echo "Country Packages"; }
		else { $isi = $city->city_name ; echo "City Packages"; }
	}
	else { $isi = $city->city_name ; echo "City Packages"; }
	 * */
//	}
    ...
    <div id="black_heading" style="width:50%"><?php echo $jdlDua; ?></div>
    <div class="detail_attractions">
    ...
    <?php 
  	  ...
  	  if(!empty($main)){
  	  	 foreach($main as $row) { 
    ?>
            <li ><img src="<?php echo $row->gambar; ?>" style="width:100%;height:100%;"></li>
     <?php  
		 }
     }
     else
 ...
 <?php 
 if($tanda == 1){$text = $text;}
 if($tanda == 2){if(is_null($text_ctr)== false){$text = $text_ctr;}else{$text;}}
 if($tanda == 3){if(is_null($text_cty)== false){$text = $text_cty;}else{$text;}}
	if(!empty($text)) {
	$limit = count($text);
		foreach($text as $row)
		{ 
...
?>
	<div id="package">
        <div id="package_list">
            <div id="package_info">		
			<div style="clear:both;"></div>
                <h1 class="red"><?php echo htmlentities($row->nama); 
                ?>
                 </h1>
                <table>
                    <tr>
                        ...
                        <?php echo htmlspecialchars_decode(htmlentities(htmlspecialchars_decode($row->ket, ENT_NOQUOTES)), ENT_NOQUOTES); ?>
                            
                        </td>
                    </tr>
                    <tr>
					<?php if(!empty($row->desc)){ 
							echo '<td colspan="3" class="red"><p><strong>***Note : </strong>'.htmlspecialchars_decode(htmlentities(htmlspecialchars_decode($row->desc, ENT_NOQUOTES)), ENT_NOQUOTES).'**</p></span></td>';
						} ?>
                    </tr>
                    <tr>
				...
				foreach($qryCekDskn->result() as $dsknRow){
					...
					/*
						$qrySlctMrgn = mysql_query("SELECT * FROM margin WHERE role_id='$userRoleID' ");
		                		...
		                		$qryGtMrgn = $this->db->get_where('margin', array('role_id' => $userRoleID));
						foreach($qryGtMrgn->result() as $mrgRow){
							$mrgInPrcnt = $mrgRow->margin_pr;
							//$mrgInNmnl = $mrgRow->margin_rp;
						}
						
		                		$prcntMrgn = $mrgInPrcnt / 100;
						$profit = $pckgsPrice * $prcntMrgn;  
						$salePrice = round($pckgsPrice + $profit, 2);
						$finalPrice = round((( $salePrice * $conv_sgd ) / $conv), 2);
						//echo $salePrice."-".$conv_sgd."-".$conv."-".$curr; 
						//$tmplHarga = 'from'.' '.$crrncy.' '.$finalPrice.' / Person';
						if($this->session->userdata('sess_conversi') != 0){	
							if($this->session->userdata('sess_currency') != ''){
								if($this->session->userdata('sess_currency') != null){
									$tmplHarga = 'from'.' '.$curr.' '.$finalPrice.' / Person';
								}else{	
									$tmplHarga = 'from'.' '.$crrncy.' '.$salePrice.' / Person';
							}}else{	
									$tmplHarga = 'from'.' '.$crrncy.' '.$salePrice.' / Person';
						}}else{	
							$tmplHarga = 'from'.' '.$crrncy.' '.$salePrice.' / Person';
						...
						$salePrice = round($pckgsPrice + $profit, 2);
						$finalPrice = round((( $salePrice * $conv_sgd ) / $conv), 2);
						$finalMarkup = round((( $priceMarkup * $conv_sgd ) / $conv), 2);
						//echo $salePrice."-".$conv_sgd."-".$conv."-".$curr; 
						//$tmplHarga = '<del>from'.' '.$crrncy.' '.number_format($priceMarkup,2).' / Person</del><br /><b>from'.' '.$crrncy.' '.number_format($salePrice,2).' / Person</b>';
						//$tmplHarga = '<del>from'.' '.$curr.' '.number_format($finalMarkup,2).' / Person</del><br /><b>from'.' '.$curr.' '.number_format($finalPrice,2).' / Person</b>';
						if($this->session->userdata('sess_conversi') != 0){	
							if($this->session->userdata('sess_currency') != ''){
								if($this->session->userdata('sess_currency') != null){
									$tmplHarga = '<del>from'.' '.$crrncy.' '.number_format($finalMarkup,2).' / Person</del><br /><b>from'.' '.$curr.' '.number_format($finalPrice,2).' / Person</b>';
								}else{	
									$tmplHarga = '<del>from'.' '.$curr.' '.number_format($priceMarkup,2).' / Person</del><br /><b>from'.' '.$curr.' '.number_format($finalPrice,2).' / Person</b>';
							}}else{	
									$tmplHarga = '<del>from'.' '.$curr.' '.number_format($priceMarkup,2).' / Person</del><br /><b>from'.' '.$curr.' '.number_format($finalPrice,2).' / Person</b>';
						}}else{	
									$tmplHarga = '<del>from'.' '.$curr.' '.number_format($priceMarkup,2).' / Person</del><br /><b>from'.' '.$curr.' '.number_format($finalPrice,2).' / Person</b>';
				...
				/*<div id="price"><?php echo "from. ".$row->currency." ".number_format($row->price, 2, '.', ''); ?> / Person </div><div class="button"><a href="<?php echo $this->config->base_url();?>index.php/detail/book/<?php echo $row->packages_id; ?>">Book now</a></div>*/
                ?>
               
                <div id="price">
                	<?php echo $tmplHarga ; ?>  
                	
                	</div><div class="button"><a href="<?php echo $this->config->base_url();?>index.php/detail/book/<?php echo $row->packages_id; ?>">Book now</a></div>
            </div>
...
<?php /*if(!empty($limit)){$limit=$limit;}else{$limit=0;} if($limit >= 10){ echo '<div id="" style="margin-left: 2%" ><a href="#"class="blue"><h1>More Packages ..</h1></a></div>'; }*/ ?>
