 ...
 */
 ...
 $trnsctnCode = $this->uri->segment(3);
 $this->db->select('*');
...
$bkngPid = $uriPid['pid']; 
*/
$qry = mysql_query("SELECT * FROM book_packages WHERE transaction_code='$trnsctnCode' ");
...
<?php } ?>
<form action="<?php echo base_url();?>index.php/booking/process" method="post" enctype="multipart/form-data" />
<?php
...
<table border="0" style="width:100%;">
	...
	<input type="hidden" name="tid" value="<?php echo $TCODE; ?>">
...
for($k=0;$k<=$totChild;$k++){
?>	
$('#txtDobChild_<?php echo $k; ?>').datepicker({
    onSelect: function(value, ui) {
        var todayChild_<?php echo $k; ?> = new Date(),
            dobChild_<?php echo $k; ?> = new Date(value),
            ageChild_<?php echo $k; ?> = new Date(todayChild_<?php echo $k; ?> - dobChild_<?php echo $k; ?>).getFullYear() - 1969;
            

        $('#txtAgeChild_<?php echo $k; ?>').val(ageChild_<?php echo $k; ?>);
    },
    maxDate: '+0d',
...
<table border="0" width="100%">
	<tr>
		<td colspan="2" style="background:#4682B4"><h2><?php if($totChild > 1){echo $j.'. )';}else{ echo '';}; ?> Child Traveler Details</h2></td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
	</tr>
	...
	<tr>
		...
		<td><?php echo form_error('txtDobChild[]', '<div style="color:red">', '</div>'); ?>
			<input type="text" id="txtDobChild_<?php echo $j; ?>" name="txtDobChild[]" class="dateOfBirth"  /></td>
	</tr>
	<!--
	<tr>
		<td>Age</td><td><?php echo form_error('txtAgeChild[]', '<div style="color:red">', '</div>'); ?>
			<input type="text" id="txtAgeChild_<?php echo $j; ?>" name="txtAgeChild[]" style="width:50px;" maxlength="2" readonly /> Years Old </td>
	</tr>-->
	<!--
	<tr>
...
<tr>
	...
	<table border="0" width=100%>
		<tr>
		<td>&nbsp;</td>
		<?php //<td align="right"><input type="submit" name="confirmBook" value="Confirm Booking" /></td>?>
		<td align="right"><button onclick="myFunction()">Confirm Booking</button>
