...
<div id="grid" class="k-grid k-widget k-secondary k-reorderable" data-role="grid" style="">
    <div class="k-grouping-header" data-role="droptarget" style="padding-left:10px;">Manage Data Packages Country</div>
    <div style="clear:both;"></div>
<!-- ===========search form start======== -->
	 <div class="pull-right" style="padding:5px; vertical-align:middle;">
     <form action="<?php echo base_url().'index.php/packages_country/searching/' ?>" enctype="multipart/form-data" method="post">
...
<!-- ===========search form end======== -->     
 ...
 <!--       <colgroup>
        ...
        <thead class="k-grid-header">
            <tr>
                <th class="k-header k-with-icon" data-title="packages Name" data-field="packagess_name" role="columnheader" data-role="sortable" style="width:40px;">
                    <a class="k-header-column-menu" href="#" tabindex="-1">
                        <span class="k-icon k-i-arrowhead-s"></span>
                    </a>
                    <a class="k-link" href="#">Country Code</a>
                </th>
...
<script>
$(document).ready(function(){
	$("#pcx<?php echo $row->idx; ?>").spinner({ min: 0 });
	$('.ui-spinner-button').click(function() { 
		$(this).siblings("#pcx<?php echo $row->idx; ?>").change(); 	
	});

	$("#pcx<?php echo $row->idx; ?>").spinner().change(function(){
             //alert($(this).spinner('value'));
            var platform_id = <?php echo $row->idx; ?>;
			var tOrder = $(this).spinner('value'); 
			$.ajax({
					type: "POST",
					url: "<?php echo site_url('packages_country/order'); ?>",
					data: {id : platform_id, content : tOrder},
			...
			<tr class="k-alt" role="row" style="text-align: center;" >
				...
				<input type="text" id="pcx<?php echo $row->idx; ?>" value="<?php echo $row->orders; ?>" name="<?php echo "order".$row->idx; ?>" style="width: 30px;">
				<!--
