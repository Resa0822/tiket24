...
<div class="content detail">
	...
	elseif($tanda == 2) { $isi = $city->country_name ; echo "Country Packages"; }
	else { $isi = $city->city_name ; echo "City Packages"; }
	?>
	</h1>
	
    <div id="black_heading" style="width:20%"><?php if(!empty($isi )){echo $isi ;} ?></div>
    ...
    <?php } }?>	
        ...
        <div class="desc">
            ...
            Nostrud nostrud quidne pneum imputo, capio quis. Feugiat valetudo praemitto molior abdo. </div>
    </div>
    <div id="black_heading" style="width:8%">Package</div>
    <div><img style=" height:3%;" src="<?php echo base_url(); ?>/asset/images/icon/flag_blue.png"> Applicablae For Cash Voucher / Discount Code</div>
 <?php 
 if($tanda == 1){$text = $text;}
 if($tanda == 2){$text = $text_ctr;}
 if($tanda == 3){$text = $text_cty;}
	if(!empty($text)) {
	$limit = count($text);
		foreach($text as $row)
		{ 
	...
	<div id="package">
        <div id="package_list">
			...
			<div style="clear:both;"></div>
                <h1 class="red"><?php echo " >> ".$row->nama; 
                if(!empty($row->discount) ){
			if($row->discount > 0){
		?><img style=" height:3%;" src="<?php echo base_url(); ?>/asset/images/icon/flag_blue.png"><font size="3"><?php if(!empty($row->discount)){echo $row->discount.' %';}?></font>
		<?php
			}
		} ?>
                 </h1>
				...
				foreach($qryCekDskn->result() as $dsknRow){
					...
					if($diskon == 0){
						...
						$tmplHarga = 'fr'. ' '.$crrncy.' '.$salePrice.' / Person';
					}
					else{
						...
						$salePrice = round($pckgsPrice + $profit, 2);
						$tmplHarga = '<del>fr'.' '.$crrncy.' '.$priceMarkup.' / Person</del><br /><b>fr'. ' '.$crrncy.' '.$salePrice.' / Person</b>';
					}
